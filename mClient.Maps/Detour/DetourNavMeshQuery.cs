...
//////////////////////////////////////////////////////////////////////////////////////////
    ...
    public class dtNavMeshQuery{
						        ...
						        ref dtPolyRef randomRef, ref float[] randomPt)
        {
	        ...
	        Detour.dtVcopy(ref verts, 0*3,tile.verts,vStart);
	        for (int j = 1; j < poly.vertCount; ++j)
	        {
		        //v = &tile.verts[poly.verts[j]*3];
		        Detour.dtVcopy(ref verts, j*3,tile.verts,poly.verts[j]*3);
	        }
	        ...
	        Detour.dtVcopy(ref randomPt, 0 , pt, 0);
	        randomRef = polyRef;
													         ...
													         ref dtPolyRef randomRef,ref float[] randomPt)
        {
	        ...
	        dtVcopy(ref startNode.pos, centerPos);
	        startNode.pidx = 0;
	        startNode.cost = 0;
	        startNode.total = 0;
	        ...
	        while (!m_openList.empty())
	        {
		        ...
		        for (uint i = bestPoly.firstLink; i != DT_NULL_LINK; i = bestTile.links[i].next)
		        {
			        ...
			        if (neighbourNode.flags == 0){
				        dtVlerp(ref neighbourNode.pos, va, vb, 0.5f);
                    }
	        ...
	        float[] verts = new float[3*DT_VERTS_PER_POLYGON];
	        float[] areas = new float[DT_VERTS_PER_POLYGON];
	        dtVcopy(ref verts, 0*3, randomTile.verts, 0);
	        for (int j = 1; j < randomPoly.vertCount; ++j)
	        {
		        //v = &randomTile.verts[randomPoly.verts[j]*3];
		        dtVcopy(ref verts, j*3,randomTile.verts,randomPoly.verts[j]*3);
	        }
        ...
        ///
		public dtStatus closestPointOnPoly(dtPolyRef polyRef, float[] pos, ref float[] closest, ref bool posOverPoly)
        {
	        Debug.Assert(m_nav != null);
	        dtMeshTile tile = null;
	        dtPoly poly = null;
	        ...
	        if (poly.getType() == (byte)dtPolyTypes.DT_POLYTYPE_OFFMESH_CONNECTION)
	        {
		        ...
		        float u = d0 / (d0+d1);
		        dtVlerp(ref closest, 0, tile.verts,v0Start, tile.verts, v1Start, u);
		        //if (posOverPoly)
			    posOverPoly = false;
		        return DT_SUCCESS;
	        }
	        ...
	        for (int i = 0; i < nv; ++i){
		        dtVcopy(ref verts, i*3, tile.verts,poly.verts[i]*3);
            }
	
	        dtVcopy(ref closest, pos);
	        if (!dtDistancePtPolyEdgesSqr(pos, 0, verts, nv, edged, edget))
	        {
		        ...
		        dtVlerp(ref closest, 0, verts,vaStart,verts, vbStart, edget[imin]);
        ...
        /// 
		public dtStatus closestPointOnPolyBoundary(dtPolyRef polyRef, float[] pos, ref float[] closest) 
        {
	        Debug.Assert(m_nav != null);
	        ...
	        {
		        dtVcopy(ref verts, nv*3, tile.verts,poly.verts[i]*3);
		        nv++;
	        }		
										         ...
										         ref dtPolyRef nearestRef, ref float[] nearestPt) 
        {
	        ...
	        int polyCount = 0;
	        if (dtStatusFailed(queryPolygons(center, extents, filter, ref polys, ref polyCount, 128)))
		        return DT_FAILURE | DT_INVALID_PARAM;
	        ...
	        for (int i = 0; i < polyCount; ++i)
	        {
		        ...
		        float d = 0;
		        closestPointOnPoly(polyRef, center, ref closestPtPoly, ref posOverPoly);

		        // If a point is directly over a polygon and closer than
		        // climb height, favor that instead of straight line nearest point.
		        dtVsub(ref diff, center, closestPtPoly);
		        if (posOverPoly)
		        {
			        dtMeshTile tile = null;
			        dtPoly poly = null;
		        ...
		        {
			        //if (nearestPt != null)
                    dtVcopy(ref nearestPt, closestPtPoly);
										        ...
										        dtPolyRef[] polys, int polyStart, int maxPolys)
        {
	        ...
	        else
	        {
		        ...
		        for (int i = 0; i < tile.header.polyCount; ++i)
		        {
			        ...
			        dtVcopy(ref bmin, 0, tile.verts,vStart);
			        dtVcopy(ref bmax, 0, tile.verts,vStart);
			        for (int j = 1; j < p.vertCount; ++j)
			        {
				        //v = &tile.verts[p.verts[j]*3];
                        vStart = p.verts[j]*3;
				        dtVmin(ref bmin, 0, tile.verts,vStart);
				        dtVmax(ref bmax, 0, tile.verts,vStart);
			        }
		...
		public dtStatus queryPolygons(float[] center, float[] extents,
									           dtQueryFilter filter,
									           ref dtPolyRef[] polys, ref int polyCount, int maxPolys)
        {
	        Debug.Assert(m_nav != null);
	
	        float[] bmin = new float[3];//, bmax[3];
            float[] bmax = new float[3];
	        dtVsub(ref bmin, center, extents);
	        dtVadd(ref bmax, center, extents);
	        ...
	        for (int y = miny; y <= maxy; ++y)
	        {
		        ...
		        {
			        int nneis = m_nav.getTilesAt(x,y, ref neis, MAX_NEIS);
		...
		public dtStatus findPath(dtPolyRef startRef, dtPolyRef endRef,
								          ...
								          dtQueryFilter filter,
								          ref dtPolyRef[] path, ref int pathCount, int maxPath)
        {
	        Debug.Assert(m_nav != null);
	        Debug.Assert(m_nodePool != null);
	        Debug.Assert(m_openList != null);
	        ...
	        dtNode startNode = m_nodePool.getNode(startRef);
	        dtVcopy(ref startNode.pos, startPos);
	        startNode.pidx = 0;
	        startNode.cost = 0;
											        ...
											        dtQueryFilter filter)
        {
	        ...
	        dtVcopy(ref m_query.startPos, startPos);
	        dtVcopy(ref m_query.endPos, endPos);
        ...
        /// @returns The status flags for the query.
		public dtStatus finalizeSlicedFindPath(ref dtPolyRef[] path, ref int pathCount, int maxPath)
        {
	        pathCount = 0;
									          ...
									          ref int straightPathCount, int maxStraightPath)
        {
	        ...
	        {
		        // Append new vertex.
		        dtVcopy(ref straightPath, straightPathCount*3, pos,0);
		        if (straightPathFlags != null)
        ...
        public dtStatus findStraightPath(float[] startPos, float[] endPos,
										          ...
										          float[] straightPath, ref byte[] straightPathFlags, ref dtPolyRef[] straightPathRefs,
										          ref int straightPathCount, int maxStraightPath, int options)
        {
	        Debug.Assert(m_nav != null);
	        ...
	        if (dtStatusFailed(closestPointOnPolyBoundary(path[0], startPos, ref closestStartPos)))
		        return DT_FAILURE | DT_INVALID_PARAM;

	        float[] closestEndPos = new float[3];
	        if (dtStatusFailed(closestPointOnPolyBoundary(path[pathSize-1], endPos, ref closestEndPos)))
		        return DT_FAILURE | DT_INVALID_PARAM;
	        ...
	        if (pathSize > 1)
	        {
		        ...
		        dtVcopy(ref portalApex, closestStartPos);
		        dtVcopy(ref portalLeft, portalApex);
		        dtVcopy(ref portalRight, portalApex);
		        int apexIndex = 0;
		        int leftIndex = 0;
		        int rightIndex = 0;
		        ...
		        for (int i = 0; i < pathSize; ++i)
		        {
			        ...
			        if (i+1 < pathSize)
			        {
				        ...
				        if (dtStatusFailed(getPortalPoints(path[i], path[i+1], left, right, ref fromType,ref toType)))
				        {
					        ...
					        if (dtStatusFailed(closestPointOnPolyBoundary(path[i], endPos, ref closestEndPos)))
					        {
			        ...
			        {
				        // End of the path.
				        dtVcopy(ref left, closestEndPos);
				        dtVcopy(ref right, closestEndPos);
			        ...
			        if (dtTriArea2D(portalApex, portalRight, right) <= 0.0f)
			        {
				        ...
				        else
				        {
					        ...
					        dtVcopy(ref portalApex, portalLeft);
					        apexIndex = leftIndex;
			        ...
			        if (dtTriArea2D(portalApex, portalLeft, left) >= 0.0f)
			        {
				        ...
				        else
				        {
					        ...
					        dtVcopy(ref portalApex, portalRight);
        ...
        public dtStatus moveAlongSurface(dtPolyRef startRef, float[] startPos, float[] endPos,
										          dtQueryFilter filter,
										          ref float[] resultPos, ref dtPolyRef[] visited, ref int visitedCount, int maxVisitedSize)
        {
	        Debug.Assert(m_nav != null);
	        ...
	        dtVcopy(ref bestPos, startPos);
	
	        // Search constraints
	        float[] searchPos = new float[3];//, searchRadSqr;
            float searchRadSqr = .0f;
	        dtVlerp(ref searchPos, startPos, endPos, 0.5f);
	        ...
	        while (nstack != 0)
	        {
		        ...
		        for (int i = 0; i < nverts; ++i){
			        dtVcopy(ref verts, i*3, curTile.verts,curPoly.verts[i]*3);
                }   
		        ...
		        for (int i = 0, j = (int)curPoly.vertCount-1; i < (int)curPoly.vertCount; j = i++)
		        {
			        ...
			        if (nneis == 0)
			        {
				        ...
				        if (distSqr < bestDist)
				        {
					        ...
					        dtVlerp(ref bestPos, 0, verts, vjStart,verts, viStart, tseg);
					        bestDist = distSqr;
					        bestNode = curNode;
				        }
			        }
										         ...
										         float[] left, float[] right)
        {
	        ...
	        if (fromPoly.getType() == (byte) dtPolyTypes.DT_POLYTYPE_OFFMESH_CONNECTION)
	        {
		        ...
		        for (uint i = fromPoly.firstLink; i != DT_NULL_LINK; i = fromTile.links[i].next)
		        {
			        if (fromTile.links[i].polyRef == to)
			        {
				        ...
				        dtVcopy(ref left, 0, fromTile.verts,fromPoly.verts[v]*3);
				        dtVcopy(ref right, 0, fromTile.verts,fromPoly.verts[v]*3);
				        return DT_SUCCESS;
			        }
		        }
		        return DT_FAILURE | DT_INVALID_PARAM;
	        ...
	        if (toPoly.getType() == (byte) dtPolyTypes.DT_POLYTYPE_OFFMESH_CONNECTION)
	        {
		        for (uint i = toPoly.firstLink; i != DT_NULL_LINK; i = toTile.links[i].next)
		        {
			        if (toTile.links[i].polyRef == from)
			        {
				        ...
				        dtVcopy(ref left, 0, toTile.verts, toPoly.verts[v]*3);
				        dtVcopy(ref right, 0, toTile.verts, toPoly.verts[v]*3);
				        return DT_SUCCESS;
			        }
		        }
		        return DT_FAILURE | DT_INVALID_PARAM;
	        ...
	        dtVcopy(ref left, 0, fromTile.verts,v0*3);
	        dtVcopy(ref right, 0, fromTile.verts,v1*3);
	        ...
	        if (link.side != 0xff)
	        {
		        ...
		        if (link.bmin != 0 || link.bmax != 255)
		        {
			        ...
			        dtVlerp(ref left, 0, fromTile.verts, v0*3, fromTile.verts, v1*3, tmin);
			        dtVlerp(ref right, 0, fromTile.verts, v0*3, fromTile.verts, v1*3, tmax);
		        }
	        }
        ...
        dtStatus raycast(dtPolyRef startRef, float[] startPos, float[] endPos,
								         dtQueryFilter filter,
								         ref float t, ref float[] hitNormal, ref dtPolyRef[] path, ref int pathCount, int maxPath)
        {
	        Debug.Assert(m_nav != null);
        ...
        dtStatus findPolysAroundCircle(dtPolyRef startRef, float[] centerPos, float radius,
											           dtQueryFilter filter,
											           ref dtPolyRef[] resultRef, ref dtPolyRef[] resultParent, ref float[] resultCost,
											          ...
											          ref int resultCount, int maxResult)
        {
	        ...
	        for (int i = 0; i < nverts; ++i){
		        dtVadd(ref centerPos, 0,centerPos,0,verts,i*3);
            }
	        dtVscale(ref centerPos, centerPos,1.0f/nverts);
        ...
        dtStatus findLocalNeighbourhood(dtPolyRef startRef, float[] centerPos, float radius,
												        dtQueryFilter filter,
												        ref dtPolyRef[] resultRef, ref dtPolyRef[] resultParent,
												        ref int resultCount, int maxResult)
        {
	        Debug.Assert(m_nav != null);
	        Debug.Assert(m_tinyNodePool != null);
	        ...
	        while (nstack != 0)
	        {
		        ...
		        for (uint i = curPoly.firstLink; i != DT_NULL_LINK; i = curTile.links[i].next)
		        {
			        ...
			        for (int k = 0; k < npa; ++k){
				        dtVcopy(ref pa, k*3, neighbourTile.verts,neighbourPoly.verts[k]*3);
                    }
			        ...
			        for (int j = 0; j < n; ++j)
			        {
				        ...
				        for (int k = 0; k < npb; ++k){
					        dtVcopy(ref pb, k*3, pastTile.verts,pastPoly.verts[k]*3);
                        }
        ...
        dtStatus getPolyWallSegments(dtPolyRef polyRef, dtQueryFilter filter,
											         ref float[] segmentVerts,ref  dtPolyRef[] segmentRefs, ref int segmentCount,
											         int maxSegments)
        {
	        Debug.Assert(m_nav != null);
	        ...
	        for (int i = 0, j = (int)poly.vertCount-1; i < (int)poly.vertCount; j = i++) {
                ...
                if ((poly.neis[j] & DT_EXT_LINK) != 0) {
                    ...
                    if (n < maxSegments) {
                        ...
                        int segStart = n * 6;
                        dtVcopy(ref segmentVerts, segStart, tile.verts, vjStart);
                        dtVcopy(ref segmentVerts, segStart + 3, tile.verts, viStart);
                        if (segmentRefs != null)
                ...
                for (int k = 1; k < nints; ++k) {
                    ...
                    if (storePortals && ints[k].polyRef != 0) {
                        ...
                        if (n < maxSegments) {
                            //float* seg = &segmentVerts[n*6];
                            int segStart = n * 6;
                            dtVlerp(ref segmentVerts, segStart, tile.verts, vjStart2, tile.verts, viStart2, tmin);
                            dtVlerp(ref segmentVerts, segStart + 3, tile.verts, vjStart2, tile.verts, viStart2, tmax);
                            if (segmentRefs != null)
											        ...
											        ref float hitDist, ref float[] hitPos, ref float[] hitNormal)
        {
	        ...
	        // Calc hit normal.
	        dtVsub(ref hitNormal, centerPos, hitPos);
	        dtVnormalize(hitNormal);
