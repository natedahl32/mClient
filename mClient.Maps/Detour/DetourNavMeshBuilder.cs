    ...
    */
    ...
    public static bool dtCreateNavMeshData(dtNavMeshCreateParams createParams, out dtRawTileData outTile)//ref byte[] outData, ref int outDataSize)
    {
	    ...
	    if (createParams.offMeshConCount > 0)
	    {
		    ...
		    dtVcopy(ref bmin, createParams.bmin);
		    dtVcopy(ref bmax, createParams.bmax);
		    bmin[1] = hmin;
		    bmax[1] = hmax;
	    ...
	    */
	    ...
	    dtVcopy(ref header.bmin, createParams.bmin);
	    dtVcopy(ref header.bmax, createParams.bmax);
	    ...
	    for (int i = 0; i < createParams.offMeshConCount; ++i)
	    {
		    ...
		    if (offMeshConClass[i*2+0] == 0xff)
		    {
			    ...
			    dtVcopy(ref navVerts, vStart + 0, createParams.offMeshConVerts, linkVStart + 0);
			    dtVcopy(ref navVerts, vStart + 3, createParams.offMeshConVerts, linkVStart + 3);
			    n++;
		    }
	    }
	    ...
	    for (int i = 0; i < createParams.offMeshConCount; ++i)
	    {
		    ...
		    if (offMeshConClass[i*2+0] == 0xff)
		    {
			    ...
			    dtVcopy(ref con.pos,0, createParams.offMeshConVerts, endPtsStart + 0);
			    dtVcopy(ref con.pos,3, createParams.offMeshConVerts, endPtsStart + 3);
