    ...
    */
    ...
    public class dtNavMesh{
        ...
        public dtStatus init(dtNavMeshParams navMeshParams)
        {
	        ...
	        dtVcopy(ref m_orig, navMeshParams.orig);
        ...
        public dtStatus init(dtRawTileData rawTile, int flags)
        {
	        ...
	        dtVcopy(ref navMeshParams.orig, header.bmin);
        ...
        public void connectExtOffMeshLinks(dtMeshTile tile, dtMeshTile target, int side)
        {
	        ...
	        for (int i = 0; i < target.header.offMeshConCount; ++i)
	        {
		        ...
		        dtVcopy(ref target.verts, vIndex, nearestPt, 0);
        ...
        public void baseOffMeshLinks(dtMeshTile tile)
        {
	        ...
	        for (int i = 0; i < tile.header.offMeshConCount; ++i)
	        {
		        ...
		        dtVcopy(ref tile.verts, vIndex, nearestPt, 0);
        ...
        public void closestPointOnPoly(dtPolyRef polyRef, float[] pos, int posStart, float[] closest, ref bool posOverPoly)
        {
	        ...
	        dtVcopy(ref closest, 0, pos, posStart);
								        ...
								        float[] nearestPt) 
        {
	        ...
	        dtVsub(ref bmin, 0, center, centerStart, extents, 0);
	        dtVadd(ref bmax, 0, center, centerStart, extents, 0);
	        ...
	        for (int i = 0; i < polyCount; ++i)
	        {
		        ...
		        dtVsub(ref diff, 0, center, centerStart, closestPtPoly, 0);
		        if (posOverPoly)
		        {
                                ...
                                dtPolyRef[] polys, int maxPolys)
        {
	        ...
	        else
	        {
		        ...
		        for (int i = 0; i < tile.header.polyCount; ++i)
		        {
			        ...
			        dtVcopy(ref bmin, 0 ,tile.verts, vIndex);
			        dtVcopy(ref bmax, 0 ,tile.verts, vIndex);
			        for (int j = 1; j < p.vertCount; ++j)
			        {
				        //v = &tile.verts[p.verts[j]*3];
                        vIndex = p.verts[j]*3;
				        dtVmin(ref bmin, 0 ,tile.verts, vIndex);
				        dtVmax(ref bmax, 0 ,tile.verts, vIndex);
			        }
        ...
        public dtStatus addTile(dtRawTileData rawTileData, int flags, dtTileRef lastRef, ref dtTileRef result)
        {
	        ...
	        nneis = getTilesAt(header.x, header.y, ref neis, MAX_NEIS);
        ...
        /// @return The status flags for the operation.
        public dtStatus getOffMeshConnectionPolyEndPoints(dtPolyRef prevRef, dtPolyRef polyRef, ref float[] startPos, ref float[] endPos)
        {
	        uint salt = 0, it = 0, ip = 0;
	        ...
	        dtVcopy(ref startPos, 0, tile.verts, poly.verts[idx0]*3);
	        dtVcopy(ref endPos, 0, tile.verts, poly.verts[idx1]*3);
